...
def _robust_peak_slope_from_log_points(
    log_points: List[Tuple[float, float, float]],
    low: float,
    high: float,
    max_delta: float,
) -> Tuple[Optional[float], Optional[float]]:
    """Return a robust reference slope from the rising log-linear region.

    A common failure mode in automatic qPCR efficiency estimation is selecting a
    very straight but late/flattening window: R2 is excellent, but slope is too
    low. This helper estimates a reference rising-phase single-cycle slope using
    adjacent positive points that are above noise and below plateau. The 80th
    percentile is used instead of the absolute maximum so a single noisy jump
    does not dominate the decision.
    """
    if len(log_points) < 2:
        return None, None
    steps: List[Tuple[float, float]] = []
    for i in range(len(log_points) - 1):
        c0, f0, l0 = log_points[i]
        c1, f1, l1 = log_points[i + 1]
        dx = c1 - c0
        if dx <= 0:
            continue
        # Prefer steps whose two endpoints are in the usable amplification range.
        in_range = (f0 > low and f1 > low and f0 < high and f1 < high)
        if not in_range:
            continue
        st = (l1 - l0) / dx
        if math.isfinite(st) and st > 0:
            steps.append(((c0 + c1) / 2.0, st))

    # Fallback: use all positive steps if dynamic-range filtering was too strict.
    if not steps:
        for i in range(len(log_points) - 1):
            c0, f0, l0 = log_points[i]
            c1, f1, l1 = log_points[i + 1]
    ...
    vals = [st for _, st in steps]
    ref = _quantile(vals, 0.80)
    if ref is None or not math.isfinite(ref) or ref <= 0:
        ref = max(vals)
    # Cycle closest to the reference slope; more stable than reporting a noisy max.
    ref_cycle = min(steps, key=lambda x: abs(x[1] - ref))[0]
...
def calc_efficiency_for_well(
    ...
    for baseline in baseline_candidates:
        ...
        high = float(plateau_fraction) * max_delta
        ...
        robust_peak_cycle, robust_peak_slope = _robust_peak_slope_from_log_points(log_points, low, high, max_delta)
        ...
        for window_n in range(min_window, max_window + 1):
            ...
            for start_idx in range(0, len(log_points) - window_n + 1):
                ...
                slope_quality_ref = robust_peak_slope
                slope_quality = (slope / slope_quality_ref) if slope_quality_ref is not None and slope_quality_ref > 0 else 1.0
                ...
                passes_slope_quality = slope_quality >= float(min_slope_quality)
                strict_ok = passes_noise_plateau and passes_e_r2 and passes_balance and passes_step_stability and passes_slope_quality
                ...
                score = (
                    520.0 * (r2 if math.isfinite(r2) else -1.0)
                    + 4.0 * window_n
                    + 120.0 * min(slope_quality, 1.20)
                    - 140.0 * max(0.0, float(min_slope_quality) - slope_quality)
                    - 70.0 * min(rel_balance, 2.0)
                    - 45.0 * min(rel_step_sd, 2.0)
                    - 25.0 * abs(center_frac - 0.30)
                    - 0.25 * min(cycles)
                )
                ...
                if not passes_slope_quality:
                    score -= 1800.0 * min(float(min_slope_quality) - slope_quality, 0.5)
                ...
                if not passes_slope_quality:
                    fail_parts.append("late_flat_window_low_slope_vs_rising_phase")
                ...
                cand = {
                    ...
                    "slope_quality_vs_peak": slope_quality,
                    "robust_peak_slope": robust_peak_slope,
                    "robust_peak_cycle": robust_peak_cycle,
    ...
    slope_quality_vs_peak = None
    if selected.get("robust_peak_slope") is not None and float(selected.get("robust_peak_slope")) > 0:
        derivative_peak_cycle = selected.get("robust_peak_cycle")
        derivative_peak_slope = selected.get("robust_peak_slope")
        slope_quality_vs_peak = float(selected["slope"]) / float(selected["robust_peak_slope"])
    elif derivative_peak_slope is not None and derivative_peak_slope > 0:
...
def calc_efficiency_for_baseline_subtracted_well(
    points: List[Tuple[float, float]],
    min_window: int = 5,
    max_window: int = 7,
    min_e: float = 1.20,
    ...
    """
    Efficiency estimate for Bio-Rad baseline-subtracted / baseline-corrected RFU.

    IMPORTANT: this function does NOT subtract or optimize baseline again.
    It assumes that the input fluorescence values are already corrected by Bio-Rad.

    Model:
        log10(F_baseline_subtracted) = slope * cycle + intercept
        E_amp_factor = 10^slope
        Efficiency_% = (E_amp_factor - 1) * 100

    This mode is intended for Bio-Rad exports displayed/exported as Baseline
    Subtracted RFU. Do not use it for No Baseline Subtraction/raw RFU.
    """
    points = sorted([(float(c), float(f)) for c, f in points if c is not None and f is not None])
    well_curve: List[Dict[str, Any]] = []
    candidate_window_rows: List[Dict[str, Any]] = []
    ...
    def empty_result(status: str, details: str, noise_val: Optional[float] = None,
                     max_delta_val: Optional[float] = None, low_val: Optional[float] = None,
                     high_val: Optional[float] = None) -> Dict[str, Any]:
        return {
            "status": status,
            ...
            "window_n": 0,
            "baseline": 0.0,
            "baseline_cycles": "Bio-Rad baseline-subtracted input; no additional subtraction",
            "noise": round(noise_val, 6) if noise_val is not None and math.isfinite(float(noise_val)) else None,
            "max_delta": round(max_delta_val, 6) if max_delta_val is not None and math.isfinite(float(max_delta_val)) else None,
            "threshold_low": round(low_val, 6) if low_val is not None and math.isfinite(float(low_val)) else None,
            "threshold_high": round(high_val, 6) if high_val is not None and math.isfinite(float(high_val)) else None,
    ...
    positive_vals = [f for f in values if f > 0 and math.isfinite(f)]
    if len(positive_vals) < min_window:
        for c, f in points:
            well_curve.append({
                "Cycle": c,
                "Raw_Fluorescence": f,
                "Baseline": 0.0,
                "Corrected_Fluorescence": f,
                "Log10_Corrected": math.log10(f) if f > 0 else None,
                "Selected_For_Window": False,
            })
        return empty_result("NO_POSITIVE_SIGNAL", "not_enough_positive_baseline_subtracted_points"), well_curve, candidate_window_rows

    early = [f for c, f in points if 3 <= c <= 10]
    if len(early) < 5:
        early = [f for _, f in points[:min(10, len(points))]]
    early_finite = [f for f in early if math.isfinite(f)]
    early_center = median(early_finite) if early_finite else 0.0
    noise = 1.4826 * mad(early_finite, early_center) if early_finite else 0.0
    if not math.isfinite(noise) or noise <= 0:
        diffs = [abs(points[i + 1][1] - points[i][1]) for i in range(min(10, len(points) - 1))]
        noise = median([d for d in diffs if math.isfinite(d)]) if diffs else 0.0
    if not math.isfinite(noise) or noise <= 0:
        noise = max(0.001 * max(abs(v) for v in positive_vals), 1e-12)

    max_delta = max(positive_vals)
    # Hard dynamic-range gates. For already baseline-subtracted data the baseline is zero.
    # A window should be above early-cycle noise and below plateau.
    low = max(6.0 * noise, 0.005 * max_delta, 1e-12)
    high = float(plateau_fraction) * max_delta

    # If signal is weak but positive, avoid rejecting all points solely because early noise is high.
    if high <= low:
        low = max(0.001 * max_delta, 1e-12)
        high = max(0.90 * max_delta, float(plateau_fraction) * max_delta)

    log_points = []
    for c, f in points:
        if f > 0 and math.isfinite(f):
            log_points.append((c, f, math.log10(f)))
        well_curve.append({
            "Cycle": c,
            "Raw_Fluorescence": f,
            "Baseline": 0.0,
            "Corrected_Fluorescence": f,
            "Log10_Corrected": math.log10(f) if f > 0 and math.isfinite(f) else None,
            "Selected_For_Window": False,
        })
    ...
    for window_n in range(min_window, max_window + 1):
        ...
        for start_idx in range(0, len(log_points) - window_n + 1):
            ...
            if not consecutive:
                candidate_window_rows.append({
                    "baseline": 0.0, "cycle_start": min(cycles), "cycle_end": max(cycles),
                    "window_cycles": window_cycles, "window_n": window_n, "status": "REJECTED_GAP",
                    "E_amp_factor": None, "Efficiency_percent": None, "r2": None,
                    "slope": None, "score": None,
                })
            ...
            in_dynamic_range = all((fc > low and fc < high) for fc in fcs)
            step_slopes = _step_slopes(cycles, logs)
            positive_steps = sum(1 for st in step_slopes if math.isfinite(float(st)) and st > 0)
            frac_positive_steps = positive_steps / max(len(step_slopes), 1)
            step_sd = _sd(step_slopes)
            rel_step_sd = (step_sd / slope) if slope > 0 else 999.0
            sl_low, sl_high, balance = _linreg_slopes_by_halves(cycles, logs)
            ...
            passes_e_r2 = (E >= min_e and E <= max_e and r2 >= good_r2)
            passes_balance = rel_balance <= 0.35
            passes_step_stability = rel_step_sd <= 0.45 and frac_positive_steps >= 0.80
            passes_slope_quality = slope_quality >= float(min_slope_quality)
            strict_ok = in_dynamic_range and passes_e_r2 and passes_balance and passes_step_stability and passes_slope_quality

            # Ranking: do not select only because R2 is maximal. Prefer an early, stable,
            # log-linear window with realistic E and not near plateau.
            score = (
                520.0 * (r2 if math.isfinite(r2) else -1.0)
                + 5.0 * window_n
                + 120.0 * min(slope_quality, 1.20)
                - 140.0 * max(0.0, float(min_slope_quality) - slope_quality)
                - 55.0 * min(rel_balance, 2.0)
                - 45.0 * min(rel_step_sd, 2.0)
                - 18.0 * abs(center_frac - 0.30)
                - 0.35 * min(cycles)
                - 8.0 * abs(E - 1.90)
            )
            if not in_dynamic_range:
                score -= 5000.0
            if not (E >= min_e and E <= max_e):
                score -= 2500.0
            if r2 < good_r2:
                score -= 1500.0 * min(good_r2 - r2, 0.2)
            if not passes_slope_quality:
                score -= 1800.0 * min(float(min_slope_quality) - slope_quality, 0.5)
    ...
    if selected is None:
        return empty_result(
            "NO_LOG_WINDOW",
            "no_candidate_window_for_baseline_subtracted_signal",
            noise_val=noise,
            max_delta_val=max_delta,
            low_val=low,
            high_val=high,
        ), well_curve, candidate_window_rows

    selected_set = set(float(c) for c in selected["cycles"])
    for row in well_curve:
        row["Selected_For_Window"] = float(row["Cycle"]) in selected_set

    derivative_peak_cycle = None
    derivative_peak_slope = None
    log_points_for_peak = [(c, math.log10(f)) for c, f in points if f > 0 and math.isfinite(f)]
    if len(log_points_for_peak) >= 2:
        slopes = []
        for i in range(len(log_points_for_peak) - 1):
            c0, l0 = log_points_for_peak[i]
    ...
    if selected.get("strict_ok"):
        status = "OK"
        self_check_status = "PASS"
        self_check_details = "strict_window_ok;baseline_subtracted_input;no_additional_baseline_subtraction"
    else:
        status = "BAD_FIT"
        self_check_status = "FAIL"
        self_check_details = selected.get("fail_details", "no_strict_window_found") + ";baseline_subtracted_input"

    result = {
        "status": status,
        "E_amp_factor": round(float(selected["E_amp_factor"]), 6),
        "Efficiency_percent": round(float(selected["Efficiency_percent"]), 3),
        "slope": round(float(selected["slope"]), 6),
        "intercept": round(float(selected["intercept"]), 6),
        "r2": round(float(selected["r2"]), 6),
        "residual_sd": round(float(selected["residual_sd"]), 8),
        "cycle_start": selected["cycle_start"],
        "cycle_end": selected["cycle_end"],
        "window_cycles": selected["window_cycles"],
...
def compute_per_well_efficiency(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    for sheet_name in sheet_list:
        ...
        for idx, well in enumerate(sorted_wells, start=1):
            ...
            if getattr(args, "amp_signal_mode", "raw") == "baseline_subtracted":
                res, _corr, _cand = calc_efficiency_for_baseline_subtracted_well(
                    curves[well],
                    ...
                    min_slope_quality=args.min_slope_quality,
            ...
            res = {"Well": norm_well, "Amp_Sheet": sheet, "Amp_Signal_Mode": getattr(args, "amp_signal_mode", "raw"), **res}
...
def build_merged_cq_efficiency(args: argparse.Namespace, eff_rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    for row in cq_rows:
        ...
        # If the user did not provide a dedicated biological-replicate column,
        # use the Bio-Rad Sample name as the biological unit. This prevents
        # empty BiologicalRep from merging independent samples that share the
        # same collapsed condition, e.g. ptfeb1/ptfeb2/ptfeb3 -> ptfeb.
        if not biorep and getattr(args, "auto_biorep_from_sample", True):
            biorep = sample
...
def analysis_map_has_sample_column(path: Optional[str]) -> bool:
    """Return True when analysis_map is sample-level, i.e. contains a raw Sample column.

    Sample-level map mode is for layouts like:
      Sample, Condition, ControlSample/ControlCondition, Analysis(optional), BiologicalRep(optional)
    where Sample is the exact Bio-Rad sample name (ptfeb1, k-2, ...),
    Condition is the final statistics group (ptfeb, k-, ...), and
    ControlSample/ControlCondition is the calibrator sample name for that sample.
    """
    if not path or not os.path.exists(path):
        return False
    rows = read_mapping_table(path)
    if not rows:
        return False
    keys = {k.strip().lower() for r in rows for k in r.keys()}
    sample_aliases = {"sample", "samplename", "sample_name", "bioradsample", "bio-rad_sample", "originalsample", "rawsample", "raw_sample"}
    return bool(keys & sample_aliases)


def _map_dataset_value(value: str) -> str:
    """Normalize dataset labels in analysis_map. Empty means any dataset."""
    return str(value or "").strip()


def apply_sample_level_analysis_map(args: argparse.Namespace, rows: List[Dict[str, Any]]) -> None:
    """Apply sample-level analysis_map directly to merged Cq/efficiency rows.

    This prevents the later relative-expression stage from guessing conditions or
    controls from sample names. The map is treated as authoritative and no row is
    silently re-labelled by regex when a matching map entry exists.

    Accepted map columns, case-insensitive:
      Sample / SampleName / OriginalSample / RawSample   exact Bio-Rad sample name
      Condition / StatsCondition / Group / Treatment     final group for R stats
      ControlSample / ControlCondition / Control / Calibrator  exact control sample name
      Analysis / Experiment / RunGroup                   optional independent normalization group
      BiologicalRep / BioRep / Replicate / Unit          optional biological unit label
      Dataset / FileIndex / Run                          optional 1-based input-file index
      Include / Use                                      optional 0/false/no/exclude to skip
    """
    if not getattr(args, "analysis_map", None) or not analysis_map_has_sample_column(args.analysis_map):
        return
    ...
    mapped_any_dataset: Dict[str, Dict[str, str]] = {}
    skipped = 0
    for i, rec in enumerate(map_rows, start=2):
        ...
        if include in {"0", "no", "false", "exclude", "skip"}:
            skipped += 1
            continue
        sample = get_mapping_value_ci(rec, ["Sample", "SampleName", "Sample_Name", "BioRadSample", "Bio-Rad_Sample", "OriginalSample", "RawSample", "Raw_Sample"])
        if not sample:
            raise RuntimeError(f"Analysis map row {i}: sample-level map requires a Sample column/value")
        condition = get_mapping_value_ci(rec, ["StatsCondition", "Condition", "Group", "Treatment", "SampleCondition"], sample)
        control = get_mapping_value_ci(rec, ["ControlSample", "ControlCondition", "Control", "Calibrator", "ReferenceCondition"])
        if not control:
            raise RuntimeError(f"Analysis map row {i}: missing ControlSample/ControlCondition for sample '{sample}'")
        analysis = get_mapping_value_ci(rec, ["Analysis", "Experiment", "AnalysisGroup", "RunGroup"])
        if not analysis:
            analysis = f"control__{control}"
        biorep = get_mapping_value_ci(rec, ["BiologicalRep", "BioRep", "Replicate", "Unit", "BiologicalUnit"], sample)
        dataset = _map_dataset_value(get_mapping_value_ci(rec, ["Dataset", "FileIndex", "Run", "PCRFile", "File"], ""))
        clean = {
            "Sample": sample,
            "Condition": condition,
            "StatsCondition": condition,
        ...
        if dataset:
            key = (dataset, normalize_key(sample))
            if key in mapped:
                raise RuntimeError(f"Analysis map row {i}: duplicate Dataset+Sample mapping for {dataset}:{sample}")
            mapped[key] = clean
        else:
            key = normalize_key(sample)
            if key in mapped_any_dataset:
                raise RuntimeError(f"Analysis map row {i}: duplicate Sample mapping for {sample}")
            mapped_any_dataset[key] = clean
    ...
    for r in rows:
        ...
        m = mapped.get((dataset, normalize_key(sample))) or mapped_any_dataset.get(normalize_key(sample))
        if not m:
            if sample:
                unmatched_samples.add(f"D{dataset}:{sample}" if dataset else sample)
            r["AnalysisMapStatus"] = "NO_MATCH"
            continue
        matched += 1
        r["AnalysisMapStatus"] = "MATCHED_SAMPLE_LEVEL"
        r["RawSample"] = sample
        # For normalization, Condition must stay sample-level, because controls like
        # k-2/k-3/k-4 are distinct calibrators. Final group for R is StatsCondition.
        r["Condition"] = m["Sample"]
        r["StatsCondition"] = m["StatsCondition"]
        r["Analysis"] = m["Analysis"]
        r["MappedControlCondition"] = m["ControlCondition"]
        r["BiologicalRep"] = m["BiologicalRep"]
    print(f"Sample-level analysis_map applied: matched Cq rows={matched}; skipped map rows={skipped}")
    if unmatched_samples:
        preview = ", ".join(sorted(unmatched_samples)[:25])
        print(f"WARNING: samples not found in sample-level analysis_map ({len(unmatched_samples)}): {preview}", file=sys.stderr)
...
def build_analysis_plan(args: argparse.Namespace, rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if getattr(args, "analysis_map", None):
        ...
        if analysis_map_has_sample_column(args.analysis_map):
            # Sample-level mode: Sample is the exact Bio-Rad sample name used for
            # normalization; Condition/StatsCondition is only the final statistics group.
            for i, rec in enumerate(map_rows, start=2):
                include = get_mapping_value_ci(rec, ["Include", "Use"], "1").lower()
                if include in {"0", "no", "false", "exclude", "skip"}:
                    continue
                sample = get_mapping_value_ci(rec, ["Sample", "SampleName", "Sample_Name", "BioRadSample", "Bio-Rad_Sample", "OriginalSample", "RawSample", "Raw_Sample"])
                control = get_mapping_value_ci(rec, ["ControlSample", "ControlCondition", "Control", "Control_Condition", "Calibrator", "ReferenceCondition"])
                analysis = get_mapping_value_ci(rec, ["Analysis", "Experiment", "AnalysisGroup", "RunGroup"])
                if not sample:
                    raise RuntimeError(f"Analysis map row {i}: missing Sample")
                if not control:
                    raise RuntimeError(f"Analysis map row {i}: missing ControlSample/ControlCondition")
                if not analysis:
                    analysis = f"control__{control}"
                ...
                for condition in [sample, control]:
            ...
            # Backward-compatible condition-level map.
            for i, rec in enumerate(map_rows, start=2):
...
def aggregate_relative_expression(args: argparse.Namespace, rows: List[Dict[str, Any]], target_mean_map: Optional[Dict[str, Optional[float]]] = None) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    unit_grouped: Dict[Tuple[str, str, str], str] = {}
    ...
    stats_condition_grouped: Dict[Tuple[str, str, str], List[str]] = {}
    for r in rows:
        target = str(r.get("Target", "")).strip()
        if not target:
            continue
        ...
        if q is None or cq is None:
            continue
        condition = str(r.get("Condition", "")).strip()
        sample_name = str(r.get("Sample", "")).strip()
        biorep_raw = str(r.get("BiologicalRep", "")).strip()
        if not biorep_raw and getattr(args, "auto_biorep_from_sample", True):
            biorep_raw = sample_name
        dataset = str(r.get("Dataset", "")).strip()
        # Keep biological units from different qPCR files separate unless the user
        # explicitly disables it. This avoids collisions when two Bio-Rad files both
        # contain wells/samples named ptfeb1, empty1, etc.
        if getattr(args, "include_dataset_in_unit", True) and dataset:
            biorep = f"D{dataset}:{biorep_raw or sample_name or condition}"
            unit_label = f"D{dataset}_{biorep_raw or sample_name or condition}"
            well_label = f"D{dataset}:{str(r.get('Well', '')).strip()}"
        else:
            biorep = biorep_raw or sample_name or condition
            unit_label = biorep
            well_label = str(r.get("Well", ""))
        key = (condition, biorep, target)
        stats_condition = str(r.get("StatsCondition", "")).strip() or stats_condition_name(condition)
        grouped.setdefault(key, []).append(float(q))
        cq_grouped.setdefault(key, []).append(float(cq))
        wells_grouped.setdefault(key, []).append(well_label)
        unit_grouped.setdefault(key, unit_label)
        ...
        if stats_condition:
            stats_condition_grouped.setdefault(key, []).append(stats_condition)
    ...
    for key, qs in grouped.items():
        ...
        agg[key] = {
            "Condition": c,
            "BiologicalRep": b,
            "Unit": unit_grouped.get(key, f"{c}|{b}" if b else c),
            "Target": t,
            "StatsCondition": _join_unique_values([{"v": v} for v in stats_condition_grouped.get(key, [])], "v") or stats_condition_name(c),
            "n_technical": len(qs),
            ...
            for biorep in bioreps:
                ...
                for target in targets:
                    ...
                    rel_rows.append({
                        ...
                        "StatsCondition": ta.get("StatsCondition") or stats_condition_name(actual_condition),
                        "BiologicalRep": biorep,
                        "Unit": ta["Unit"],
                        "Reference": reference,
...
def build_rstudio_input_rows(rel_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in rel_rows:
        ...
        stats_condition = str(r.get("StatsCondition", "")).strip() or stats_condition_name(condition_original)
...
def build_sample_target_efficiency_qc_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in rows:
        ...
        condition = str(r.get("Condition", "")).strip()
        sample_name = str(r.get("Sample", "")).strip()
        biorep = str(r.get("BiologicalRep", "")).strip() or sample_name
        dataset = str(r.get("Dataset", "")).strip()
        if dataset:
            biorep = f"D{dataset}:{biorep or condition}"
    ...
    for (condition, biorep, target), sub in sorted(grouped.items(), key=lambda x: (x[0][2].lower(), x[0][0].lower(), x[0][1].lower())):
        ...
        mapped_stats = ""
        for rr in sub:
            mapped_stats = str(rr.get("StatsCondition", "")).strip()
            if mapped_stats:
                break
        out.append({
            "condition": condition,
            "stats_condition": mapped_stats or stats_condition_name(condition),
...
def main() -> int:
    ...
    ap.add_argument("--min-slope-quality", type=float, default=0.55, help="Reject windows whose slope is too low relative to the robust rising-phase slope. Prevents selecting late flat high-R2 windows. Default: 0.55")
    ap.add_argument("--plateau-fraction", type=float, default=0.75, help="Upper dynamic-range gate as a fraction of max corrected fluorescence. Lower values avoid plateau/flattening windows. Default: 0.75")
    ap.add_argument("--amp-signal-mode", choices=["raw", "baseline_subtracted"], default="raw", help="Type of amplification RFU input. raw = No Baseline Subtraction/raw RFU; script subtracts baseline. baseline_subtracted = Bio-Rad Baseline Subtracted RFU; script does NOT subtract baseline again. Default: raw")
    ...
    ap.add_argument("--auto-biorep-from-sample", dest="auto_biorep_from_sample", action="store_true", default=True, help="If --biological-rep-col is not set, use Sample as biological replicate ID. Default: on")
    ap.add_argument("--no-auto-biorep-from-sample", dest="auto_biorep_from_sample", action="store_false", help="Disable automatic biological replicate ID from Sample.")
    ap.add_argument("--include-dataset-in-unit", dest="include_dataset_in_unit", action="store_true", default=True, help="Prefix biological unit/sample IDs with dataset number when multiple --amp/--cq files are combined. Default: on")
    ap.add_argument("--no-include-dataset-in-unit", dest="include_dataset_in_unit", action="store_false", help="Do not prefix biological unit/sample IDs with dataset number.")
    ...
    args = ap.parse_args()
    print("Script version: MULTI_ANALYSIS16_analysis_map_fix_2026-06-28")
    print(f"Amplification signal mode: {args.amp_signal_mode}")
    print(f"Window selection: good_r2={args.good_r2}, min_slope_quality={args.min_slope_quality}, plateau_fraction={args.plateau_fraction}")
    if args.amp_signal_mode == "baseline_subtracted":
        print("Using Bio-Rad baseline-subtracted RFU: no additional baseline subtraction will be applied for efficiency.")
    else:
        print("Using raw/no-baseline-subtraction RFU: script will estimate and subtract baseline internally.")
    ...
    # Apply sample-level analysis_map after all datasets are combined, so map rows
    # can optionally distinguish Dataset/FileIndex while still supporting sample-only maps.
    apply_sample_level_analysis_map(args, merged_rows)
    ...
    if args.write_intermediate:
        ...
        eff_fields = [
            "Dataset", "Amp_File", "Cq_File", "Well", "Amp_Sheet", "Amp_Signal_Mode",
        ...
        if args.debug:
            ...
            merged_fields = [
                "Dataset", "Amp_File", "Cq_File", "Well", "Amp_Sheet", "Fluor", "Target", "Sample", "RawSample", "Condition", "StatsCondition", "BiologicalRep", "Analysis", "MappedControlCondition", "AnalysisMapStatus", "Content",
    ...
    if args.reference:
        ...
        # Console QC: number of biological units per target x stats condition.
        rep_counts: Dict[Tuple[str, str], int] = {}
        rep_samples: Dict[Tuple[str, str], List[str]] = {}
        for rr in r_input_rows:
            if rr.get("relative_expression") is None or rr.get("log2_relative_expression") is None:
                continue
            k = (str(rr.get("target", "")), str(rr.get("condition", "")))
            rep_counts[k] = rep_counts.get(k, 0) + 1
            rep_samples.setdefault(k, []).append(str(rr.get("sample", "")))
        for k, nrep in sorted(rep_counts.items()):
            if nrep < 3:
                print(f"WARNING: rstats_input has n={nrep} biological units for target={k[0]}, condition={k[1]}: {', '.join(rep_samples.get(k, []))}", file=sys.stderr)
        ...
        if args.write_intermediate:
            write_csv(prefix + "_final_relative_expression_by_unit.csv", rel_rows, [
                "Analysis", "Target", "Condition", "StatsCondition", "BiologicalRep", "Unit", "Reference", "control_condition",
